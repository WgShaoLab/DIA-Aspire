...
def lowess_iso(x, y, lowess_frac):
    """
    Perform LOWESS smoothing with isotonic regression
    """
...
def lowess2(run, reference_run, xcol, ycol, lowess_frac, psm_fdr_threshold, min_peptides):
    """
    Perform RT alignment using lowess regression
    """
    ...
    msg = f'Info: Peptide overlap between SysteMHC and reference: {dfm.shape[0]}.'
    timestamped_echo(msg)
    ...
    if dfm.shape[0] < 50:  # use linear regression for small reference size
        ...
        # 使用一个变量接收model_info，避免未定义的变量filename
        model_info = "auto" if lowess_frac == 0 else f"manual (frac={lowess_frac})"
        
        if lowess_frac == 0:
            run[ycol] = lowess_iso_predictor(model_info, dfm[xcol].to_numpy(), dfm[ycol].to_numpy(), run[xcol].to_numpy())
        else:
            run[ycol] = lowess_iso(dfm[xcol].to_numpy(), dfm[ycol].to_numpy(), lowess_frac)(run[xcol].to_numpy())
