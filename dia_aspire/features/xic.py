...
class XICFeatureGenerator(BaseFeatureGenerator):
    ...
    DEFAULT_FRAG_TYPES: ClassVar[list[str]] = ["b_z1", "y_z1"]
    ...
    def __init__(
        ...
        ms2_match_config : Optional[MS2MatchConfig], optional
            MS2 matching config (deprecated).
        enable_smoothing : bool, optional
            Whether to apply Savitzky-Golay smoothing to XICs.
    ...
    def feature_names(self) -> list[str]:
        return [
            ...
            "total_xic_intensity",
            "max_xic_intensity",
            # NEW: co-elution and consistency features
            "co_elution_score",
    ...
    def generate(
        ...
        init_features = pd.DataFrame(
            0.0,
            index=psm_df.index,
            columns=self.feature_names,
            dtype=np.float32
        )
        psm_df = pd.concat([psm_df, init_features], axis=1)
    ...
    def _calculate_xic_features(
        ...
        Returns array of 37 features (was 35).
        """
        ...
        if self.enable_smoothing and intensity_matrix.shape[1] >= self.smoothing_window:
            smoothed_matrix = np.zeros_like(intensity_matrix)
            win = min(self.smoothing_window, intensity_matrix.shape[1])
            if win % 2 == 0:
                win -= 1
            if win >= 3:
                for i in range(intensity_matrix.shape[0]):
                    ...
                    if np.max(row) > 0:
                        smoothed_matrix[i] = savgol_filter(row, window_length=win, polyorder=self.smoothing_polyorder)
                    else:
                        smoothed_matrix[i] = row
                intensity_matrix = smoothed_matrix
        ...
        # New co-elution features
        co_elution_score = float(_calc_co_elution_score(intensity_matrix))
        main_peak_consistency = float(_calc_main_peak_consistency(intensity_matrix))
        ...
        all_features = (
            ...
            + peak_shape_features
            + (co_elution_score, main_peak_consistency)
        )

        return np.array(all_features, dtype=np.float32)


# --- Helper Functions (unchanged or enhanced) ---
...
@numba.njit
def _calc_cos_corr(array_1: np.ndarray, array_2: np.ndarray) -> float:
...
@numba.njit
def _calc_pearson_corr(array_1: np.ndarray, array_2: np.ndarray) -> float:
...
def _filter_fragments_by_intensity(
    ...
    edge_size = max(1, n_specs // 10)
    baseline_left = np.mean(intensity_matrix[:, :edge_size], axis=1)
    baseline_right = np.mean(intensity_matrix[:, -edge_size:], axis=1)
    baseline = (baseline_left + baseline_right) / 2.0
    snr = max_intensities / (baseline + 1e-8)

    valid_mask = (max_intensities >= global_max * min_intensity_ratio) & (snr >= snr_threshold)
    ...
    final_indices = valid_indices[sorted_idx[:n_to_keep]]
...
@numba.njit
def _extract_top_k_corr_features(corr_matrix: np.ndarray) -> tuple:
    ...
    def get_stats(arr, k):
        top = arr[:min(k, len(arr))]
        return float(np.median(top)), float(np.mean(top))

    t3_med, t3_mean = get_stats(sorted_corr, 3)
    t6_med, t6_mean = get_stats(sorted_corr, 6)
    t12_med, t12_mean = get_stats(sorted_corr, 12)

    return (t3_med, t3_mean, t6_med, t6_mean, t12_med, t12_mean)


def _calculate_peak_shape_features(intensity_matrix: np.ndarray) -> tuple:
...
def _calc_peak_fwhm(xic: np.ndarray) -> float:
    ...
    return float(np.sum(above_half_max))
...
def _calc_peak_base_width(xic: np.ndarray) -> float:
    ...
    return float(np.sum(xic >= threshold))
...
def _calc_peak_tailing_factor(xic: np.ndarray) -> float:
    ...
    for i in range(apex_idx, len(xic)):
        if xic[i] >= threshold:
            ...
            break
    return float(right_width) / float(left_width) if left_width > 0 else 1.0


@numba.njit
def _calc_co_elution_score(intensity_matrix: np.ndarray) -> float:
    """Lower is better."""
    n_ions, n_specs = intensity_matrix.shape
    if n_ions < 2 or n_specs < 3:
        return 1.0

    apex_positions = []
    for i in range(n_ions):
        if np.max(intensity_matrix[i]) > 0:
            apex_positions.append(np.argmax(intensity_matrix[i]))

    if len(apex_positions) < 2:
        return 1.0

    std_apex = np.std(np.array(apex_positions))
    return std_apex / (n_specs - 1)


@numba.njit
def _calc_main_peak_consistency(intensity_matrix: np.ndarray) -> float:
    ...
    for i in range(n_ions):
        if np.max(intensity_matrix[i]) > 0:
            apex_rts.append(np.argmax(intensity_matrix[i]))

    if len(apex_rts) < 2:
        return 1.0

    apex_rts = np.array(apex_rts)
    mean_rt = np.mean(apex_rts)
    if mean_rt == 0:
        return 1.0
    return 1.0 - (np.std(apex_rts) / mean_rt)
