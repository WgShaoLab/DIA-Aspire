...
class Percolator_ML:
    ...
    def explain_model_with_shap(self, df: pd.DataFrame):
        ...
        # 准备训练数据
            ...
            def silent_call(func):
                """Execute func with stdout/stderr redirected to /dev/null and ensure files are closed."""
    ...
    def _visualize_shap(self, shap_values, explain_df):
        ...
        try:
            ...
            plt.savefig(self.outpath / f"{self.percolator_model}_shap_feature_importance.png",dpi=300)
            plt.savefig(self.outpath / f"{self.percolator_model}_shap_feature_importance.pdf",dpi=300)
            plt.close()
            ...
            plt.savefig(self.outpath/ f"{self.percolator_model}_shap_summary_plot.png",dpi=300)
            plt.savefig(self.outpath/ f"{self.percolator_model}_shap_summary_plot.pdf",dpi=300)
            plt.close()
...
# 在 Percolator_ML 类中添加以下方法

    def _cv_score_topk(self, df: pd.DataFrame, top_ratio: float = 0.3) -> pd.DataFrame:
        """
        使用ml_score排序选择top-k%作为训练集，并保持target/decoy平衡
        
        Args:
            df: 输入数据框
            top_ratio: 选择比例，例如0.3表示前30%
        """
        if df.empty:
            return df
        ...
        for i in range(self.cv_fold):
            ...
            cv_df_target = df_target[t_mask]  # 2/3候选集
            test_t_df = df_target[_slice]     # 1/3测试集
            ...
            cv_df_decoy = df_decoy[d_mask]    # 2/3候选集
            test_d_df = df_decoy[_slice]      # 1/3测试集
            
            # ===== 基于ml_score选择top X% =====
            # 按ml_score降序排序
            cv_df_target_sorted = cv_df_target.sort_values('ml_score', ascending=False)
            cv_df_decoy_sorted = cv_df_decoy.sort_values('ml_score', ascending=False)
            
            # 计算各自的top k数量
            top_k_target = int(len(cv_df_target) * top_ratio)
            top_k_decoy = int(len(cv_df_decoy) * top_ratio)
            
            # 选择top k
            train_t_candidates = cv_df_target_sorted.iloc[:top_k_target]
            train_d_candidates = cv_df_decoy_sorted.iloc[:top_k_decoy]
            
            # ===== 平衡训练集 =====
            # 取两者中较小的数量，确保平衡
            min_samples = min(len(train_t_candidates), len(train_d_candidates))
            
            # 如果样本数量超过max_train_sample，限制为max_train_sample
            min_samples = min(min_samples, self.max_train_sample)
            
            # 确保至少有min_train_sample个样本
            if min_samples < self.min_train_sample:
                logging.warning(
                    f"[Rescore] Fold {i}: Only {min_samples} samples available "
                    f"(< {self.min_train_sample}). Using all available samples."
                )
                min_samples = max(min_samples, 
                                min(self.min_train_sample, 
                                    len(train_t_candidates), 
                                    len(train_d_candidates)))
            
            # 最终的平衡训练集
            train_t_df = train_t_candidates.iloc[:min_samples]
            train_d_df = train_d_candidates.iloc[:min_samples]
            
            logging.info(
                f"[Rescore] Fold {i}: "
                f"Target候选={len(cv_df_target)}, top {top_ratio*100}%={top_k_target}, "
                f"最终使用={len(train_t_df)} | "
                f"Decoy候选={len(cv_df_decoy)}, top {top_ratio*100}%={top_k_decoy}, "
                f"最终使用={len(train_d_df)}"
            )
