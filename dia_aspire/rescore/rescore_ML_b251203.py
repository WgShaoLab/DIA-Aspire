...
class CNNModel(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_channels=64, kernel_size=3, dropout=0.2):
        ...
        # 计算第一层卷积后的维度
        # conv1_out = conv_output_size(input_dim, kernel_size, padding=kernel_size//2)
        ...
        # conv2_out = conv_output_size(conv1_out, kernel_size, padding=kernel_size//2)
        ...
        # conv3_out = conv_output_size(conv2_out, kernel_size, padding=kernel_size//2)
...
class RescoreModelProvider:
    def __init__(self):
        self.model_dict = {}
        self.model_dict["linear"] = NNLinear
    ...
    def get_model(self, model_name, input_dim, **kwargs):
        if model_name.lower() not in self.model_dict:
            ...
            if model_name.lower() == "cnn":
                ...
                return self.model_dict[model_name.lower()](input_dim, **default_kwargs)
            else:
...
class NNLinearRescore:
    def __init__(self, num_features, nn_model_type="linear", **model_kwargs):
        ...
        for epoch in range(self.epoch):
            ...
            if avg_val_loss < best_val_loss:
                ...
                if patience_counter >= self.patience:
                    logging.info(f"[{self.nn_model_type.upper()}] Early stopping at epoch {epoch+1}")
                    break
    ...
    def decision_function(self, features):
        ...
        with torch.no_grad():
            for i in range(0, len(features), self.predict_batch_size):
                ...
                # outputs[i:i+self.predict_batch_size] = pred.cpu().numpy()
        return outputs
...
class Percolator_ML:
    ...
    def init_percolator_model(
        ...
        if percolator_backend.lower() == "pytorch":
            ...
            self.model = RandomForestClassifier(n_estimators=100,
                                                random_state=42,
                                                n_jobs=-1)
        elif percolator_model == "svm":
            self.model = SVR()
        elif percolator_model == "svr":
            ...
            logging.info(
                ...
                "implemented, switch to sklearn 'random_forest' model."
            )
            self.model = LinearSVR(epsilon=0.1,C=1,dual=False,
                                             max_iter=5000,
                                             random_state=42,
                                             loss='squared_epsilon_insensitive')
            self.percolator_model = "svr"
    ...
    def _predict(self, test_df):
        ...
        elif self.percolator_model == "linear":
            test_df["ml_score"] = self.model.decision_function(
                test_df[self.feature_list].values
            )

        elif self.percolator_model == "svr":
            test_df["ml_score"] = self.model.predict(
                test_df[self.feature_list].values
            )
        elif self.percolator_model == "random_forest":
            ...
            try:
                ...
                with contextlib.redirect_stdout(open(os.devnull, 'w')), \
                    ...
                    if self.percolator_backend == "pytorch":
                        ...
                        logging.info("[SHAP] using KernelExplainer to explain svm-based model")
