...
class Percolator_ML:
    ...
    def _cv_score_topk(self, df: pd.DataFrame, top_ratio: float = 0.3) -> pd.DataFrame:
        ...
        策略说明:
        1. 按decoy分组，各组内按ml_score降序排序
        2. 取前X%作为高质量候选集(HQ)，剩余为低质量集(LQ)
        3. 3-fold CV:
           - 训练集: HQ的2/3
           - 测试集: HQ的1/3 + LQ的1/3
        4. 确保3次CV后所有数据都被测试过
        ...
        df_target_hq = df_target_sorted.iloc[:n_target_hq].copy()  # 高质量集
        df_target_lq = df_target_sorted.iloc[n_target_hq:].copy()  # 低质量集
        ...
        df_decoy_hq = df_decoy_sorted.iloc[:n_decoy_hq].copy()  # 高质量集
        df_decoy_lq = df_decoy_sorted.iloc[n_decoy_hq:].copy()  # 低质量集
        ...
        for i in range(self.cv_fold):
            ...
            train_t_hq = df_target_hq[hq_t_mask]  # HQ的2/3用于训练
            test_t_hq = df_target_hq[~hq_t_mask]  # HQ的1/3用于测试
            ...
            train_d_hq = df_decoy_hq[hq_d_mask]  # HQ的2/3用于训练
            test_d_hq = df_decoy_hq[~hq_d_mask]  # HQ的1/3用于测试
            
            # ===== 为低质量集创建CV mask (仅用于测试) =====
            # Target LQ
            lq_t_mask = np.zeros(len(df_target_lq), dtype=bool)
            lq_t_slice = slice(i, len(df_target_lq), self.cv_fold)
            lq_t_mask[lq_t_slice] = True
            
            test_t_lq = df_target_lq[lq_t_mask]  # LQ的1/3用于测试
            ...
            test_d_lq = df_decoy_lq[lq_d_mask]  # LQ的1/3用于测试
